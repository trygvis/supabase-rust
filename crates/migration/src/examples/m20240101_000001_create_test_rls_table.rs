...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        // --- 2. Enable RLS ---
        let enable_sql = enable_rls_sql(&TestRlsTable::Table.to_string(), Some("public"));
        let enable_stmt = Statement::from_string(manager.get_database_backend(), enable_sql);
        manager.get_connection().execute(enable_stmt).await?;
        ...
        let select_policy = RlsPolicy {
            ...
            using: "auth.uid() = owner_id".to_string(), // Fix: Removed extra quotes
            check: None,
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        // let disable_sql = disable_rls_sql(&TestRlsTable::Table.to_string(), Some("public"));
