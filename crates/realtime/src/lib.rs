...
impl RealtimeClient {
    ...
    fn connect(
        ...
        async move {
            ...
            url.query_pairs_mut()
                .append_pair("apikey", &client_clone.key);
                                ...
                                Message::Text(text) => {
                                        ...
                                        serde_json::from_str::<serde_json::Value>(&text)
                                    {
                                        ...
                                        // チャネルメッセージ
                                            ...
                                            if let Ok(channels_guard) =
                                                reconnect_fn.channels.try_read()
                                            {
                                                if let Some(channel) = channels_guard.get(topic) {
                                                    if let Ok(callbacks_guard) =
                                                        channel.callbacks.try_read()
                                                    {
            ...
            *socket_guard = Some(tx.clone());
            ...
            let is_manually_closed = Arc::new(AtomicBool::new(
                client_clone.is_manually_closed.load(Ordering::SeqCst),
            ));
    ...
    fn reconnect(&self) -> impl std::future::Future<Output = ()> + Send + 'static {
        let client_clone = self.clone();
        async move {
            if !client_clone.options.auto_reconnect
                || client_clone.is_manually_closed.load(Ordering::SeqCst)
            {
                return;
            }
            ...
            // 最大再接続試行回数をチェック
            let current_attempt = client_clone
                .reconnect_attempts
            ...
            let factor = client_clone
                .options
                .reconnect_backoff_factor
                .powi(current_attempt as i32 - 1);
            let interval = (base_interval * factor)
                .min(client_clone.options.max_reconnect_interval as f64)
                as u64;
...
impl<'a> ChannelBuilder<'a> {
        ...
        F: Fn(PresenceChange) + Send + Sync + 'static,
    {
        ...
        let presence_callback = move |payload: Payload| {
            if let Ok(presence_diff) =
                serde_json::from_value::<PresenceChange>(payload.data.clone())
            {
        ...
        self.callbacks
            .insert("presence".to_string(), Box::new(presence_callback));
        self
    }
    ...
    pub async fn subscribe(self) -> Result<Subscription, RealtimeError> {
        ...
        if let Some(socket) = &*socket_guard {
            ...
            socket.send(message).await.map_err(|e| {
                RealtimeError::SubscriptionError(format!("Failed to send join message: {}", e))
            })?;
    ...
    pub async fn track_presence(
        ...
        if let Some(tx) = &*socket_guard {
            ...
            tx.send(Message::Text(presence_msg.to_string()))
                .await
                .map_err(|_| {
                    RealtimeError::ChannelError("Failed to send presence message".to_string())
                })?;
...
impl Channel {
    ...
    async fn unsubscribe(&self, id: &str) -> Result<(), RealtimeError> {
        ...
        if callbacks_guard.is_empty() {
            ...
            if let Some(tx) = &*socket_guard {
                tx.send(Message::Text(unsubscribe_message.to_string()))
                    .await
                    .map_err(|_| {
                        RealtimeError::SubscriptionError(
                            "Failed to send unsubscription message".to_string(),
                        )
                    })?;
            } else {
                return Err(RealtimeError::ConnectionError(
