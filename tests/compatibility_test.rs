...
async fn test_postgrest_api_compatibility() {
    ...
    assert!(users_result.is_ok(), "from().select().execute()でデータ取得できること");
    
    if let Ok(users) = users_result {
        assert_eq!(users.len(), 2, "2つのユーザーレコードが取得できること");
        assert_eq!(users[0]["name"], "Alice", "最初のユーザーの名前がAliceであること");
    }
...
async fn test_live_compatibility() {
    ...
    assert!(tables_result.is_ok(), "システムテーブルからデータ取得できること");
