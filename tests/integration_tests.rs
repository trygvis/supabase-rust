...
async fn test_storage_realtime_postgrest_integration() {
    ...
    // 1. 認証テスト: テストユーザーを作成
    println!("\n== 認証テスト ==");
    let auth_result = supabase.auth().sign_up(&test_email, "test_password123").await;
    ...
    match auth_result {
        Ok(auth_response) => {
            ...
            
            {
                ...
                match task_result {
                    Ok(task) => {
                        println!("タスク作成成功: {}", task[0]["id"]);
                        let task_id = task[0]["id"].as_i64().unwrap();
                        ...
                        match channel_result {
                            Ok(_) => {
                                ...
                                let update_postgrest = supabase.from("tasks").with_auth(&auth_token).unwrap();
                                ...
                                match update_result {
                                    Ok(_) => println!("タスク更新成功 - リアルタイム更新が送信されます"),
                        ...
                        // クリーンアップ: タスクを削除
                        // 新しいPostgRESTクライアントを作成して使用
                        let delete_postgrest = supabase.from("tasks").with_auth(&auth_token).unwrap();
